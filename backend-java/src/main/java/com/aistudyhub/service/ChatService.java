...
public class ChatService {
    ...
    private PythonContextDocument toPythonContextDocument(SummaryHit hit) {
        String documentContent = hit.summaryContent();
        try {
            documentContent = documentService.getAiReadableText(hit.documentId());
        } catch (Exception e) {
            log.warn("Could not load full document content for AI chat, using summary instead. documentId={}, reason={}",
                    hit.documentId(), e.getMessage());
        }

        return new PythonContextDocument(
                hit.documentId(), hit.documentName(), hit.title(),
                hit.subjectId(), hit.subjectName(), hit.score(), documentContent
        );
    }
