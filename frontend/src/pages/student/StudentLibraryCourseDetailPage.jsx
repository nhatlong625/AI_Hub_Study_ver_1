...
// Check trước ở FE (dựa vào updatedAt đã có sẵn trong DocumentResponse) để
// chặn request spam lên BE trong lúc còn cooldown, không phải đợi BE trả 429
// mới biết. BE vẫn là nguồn sự thật cuối cùng — check này chỉ là tối ưu UX.
...
// ── Notice modal — thay cho alert() thô, dùng cho cooldown/409 ──
...
// ── Toggle component — 3 trạng thái ─────────────────────────
// PRIVATE  → toggle OFF  (gray)
// PENDING  → toggle OFF  (amber) — vẫn bấm được để TỰ HỦY request, quay về Private
// PUBLIC   → toggle ON   (indigo)
...
export default function StudentLibraryCourseDetailPage() {
  ...
  const userId = 1;
  // track doc IDs đang trong quá trình toggle (để disable tránh double-click)
  const [togglingIds, setTogglingIds] = useState(new Set());
  // { title, message } — hiện VisibilityNoticeModal khi bị cooldown/409
  ...
  // ── Toggle visibility (3 trạng thái) ──────────────────────
  async function handleToggleVisibility(doc) {
    if (togglingIds.has(doc.documentId)) return;

    let newStatus;
    if (doc.visibilityStatus === VIS.PUBLIC) {
      // PUBLIC → PRIVATE: tức thì, không cần admin
      newStatus = VIS.PRIVATE;
    } else if (doc.visibilityStatus === VIS.PRIVATE) {
      // PRIVATE → PENDING_REVIEW: gửi yêu cầu admin duyệt
      newStatus = VIS.PENDING;
    } else if (doc.visibilityStatus === VIS.PENDING) {
      // PENDING → PRIVATE: tự hủy request trước khi admin duyệt
      newStatus = VIS.PRIVATE;
    } else {
      return;
    }

    // Check cooldown 1h ngay ở FE trước khi gọi BE — chặn việc bấm liên tục
    // bắn nhiều request lên server trong lúc chắc chắn sẽ bị 429. BE vẫn là
    // nguồn sự thật cuối cùng (case bên dưới vẫn xử lý 429 nếu lệch giờ/đa tab).
    ...
    try {
      ...
      // Dùng err.status (libraryApi.js đã set từ HTTP status code) — đáng tin
      // cậy hơn đoán substring trong err.message, vì BE trả message tiếng
      // Việt thuần (không chứa "429"/"cooldown") nên cách đoán cũ luôn fail.
      // Hiện thẳng err.message từ BE để không phải đồng bộ tay nội dung/thời
      // lượng cooldown ở 2 nơi (BE đổi gì, FE tự đúng theo).
      if (err?.status === 429) {
  ...
  // Gọi BE thật (PATCH /documents/{id}/title). Không bắt lỗi ở đây — để lỗi
  // throw lên cho EditDocumentModal tự catch + hiện inline, giữ modal mở để
  // user sửa/thử lại. Không tự setEditDoc(null): modal tự gọi onClose() khi
  // onSave resolve thành công.
...
// ── Upload Modal (PRIVATE hardcode, không có dropdown visibility) ───
function LibraryUploadModal({ subjectId, onClose, onUploaded }) {
  ...
  function handleUpload() {
    ...
    setUploading(true);
    // visibilityStatus luôn là "PRIVATE" — hardcode
    documentApi
      .upload(file, title.trim(), subjectId, userId, "PRIVATE")
      .then((newDoc) => {
        onUploaded?.(newDoc);
        setUploading(false);
      })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-2xl w-full max-w-[540px] mx-4 shadow-2xl overflow-hidden">
        ...
        <div className="px-7 py-6 flex flex-col gap-5">
          ...
          {/* Private notice — thay cho dropdown */}
