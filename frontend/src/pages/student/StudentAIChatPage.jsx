...
function StudentAIChatPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([ownDocuments, sharedDocuments]) => {
        ...
        [...(Array.isArray(ownDocuments) ? ownDocuments : []), ...normalizedShared].forEach(doc => {
          if (doc?.documentId) documentsById.set(doc.documentId, doc);
        });
        setLibraryDocuments([...documentsById.values()]);
      })
      .catch(() => {
        setLibraryDocuments([]);
      });
  ...
  const handleSendMessage = (text) => {
    ...
    askAiChat({
      ...
      documentIds: referencedDocumentId ? [referencedDocumentId] : [],
      message: text,
      topK: 1
    })
      .then(response => {
