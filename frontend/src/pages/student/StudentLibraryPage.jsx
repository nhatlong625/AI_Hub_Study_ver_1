...
// ── CreateCourseModal ────────────────────────────────────────
// Cho user chọn Semester → chọn môn từ danh sách hệ thống → Create
function CreateCourseModal({
  ...
  const [step, setStep] = useState(1); // 1: chọn semester, 2: chọn môn
  ...
  // Danh sách môn của semester đã chọn, lọc ra những môn user chưa add
  ...
  async function handleCreate() {
    ...
    setCreating(true);
    try {
      // Không tạo subject mới trong DB hệ thống — chỉ ghi nhận user đã add
      // môn này vào Library cá nhân (USER_SUBJECT), rồi navigate vào folder.
      await onCreated({
        ...
        <div className="flex items-center justify-between px-7 pt-6 pb-4 border-b border-gray-100">
          <div>
            ...
            <h2 className="text-xl font-black text-gray-900">
              {step === 1
                ? "Select Semester"
                : `Add course — ${selectedSem?.semesterName}`}
            </h2>
...
export default function StudentLibraryPage() {
  ...
  // allSemesters: toàn bộ semester + subject của hệ thống (để chọn khi Create Course)
  const [allSemesters, setAllSemesters] = useState([]);
  // addedSubjects: subject user đã "add" vào Library — nguồn thật từ USER_SUBJECT,
  // tồn tại độc lập với việc đã upload doc hay chưa
  const [addedSubjects, setAddedSubjects] = useState([]); // [{ subjectId, addedAt }]
  // docCounts: { [subjectId]: số file } — chỉ dùng để hiện số liệu, không quyết định subject có hiện hay không
  const [docCounts, setDocCounts] = useState({});
  // docStorage: { [subjectId]: tổng bytes } — dùng để trừ đúng khi xóa 1 subject khỏi Library
  const [docStorage, setDocStorage] = useState({});
  const [loading, setLoading] = useState(true);
  const [showCreate, setShowCreate] = useState(false);
  const [confirmDialog, setConfirmDialog] = useState(null);
  // actionMenu: { semesterId, step } — step: "root" (chỉ có nút Delete) | "subjects" (list môn để chọn xóa)
  ...
  useEffect(() => {
    // Load toàn bộ semester+subject của hệ thống
    semesterApi.getAll().then((data) => {
      setAllSemesters(data);
    });

    // Load song song: subject đã add (USER_SUBJECT) + document để tính docCount/storage
    Promise.all([
      userSubjectApi.getByUser(userId),
      ...
      .then(([links, docs]) => {
        ...
        docs.forEach((doc) => {
          files++;
          storage += doc.documentSize ?? 0;
          counts[doc.subjectId] = (counts[doc.subjectId] ?? 0) + 1;
          storageBySubject[doc.subjectId] =
            (storageBySubject[doc.subjectId] ?? 0) + (doc.documentSize ?? 0);
        });
        setDocCounts(counts);
        setDocStorage(storageBySubject);
        setTotalFiles(files);
        setTotalStorageBytes(storage);
      })
      .finally(() => setLoading(false));
  }, [userId]);

  // userSubjects: enrich addedSubjects với tên (từ allSemesters) + docCount (từ docCounts)
  ...
  // Luôn render đủ Semester 0 → Semester 9 (kể cả rỗng) — subjects của mỗi
  // semester chỉ gồm những môn user đã add (có doc), không phải toàn bộ môn hệ thống.
  ...
  const storagePercent = Math.min(
    100,
    (totalStorageBytes / (10 * 1024 * 1024 * 1024)) * 100,
  ).toFixed(1);
  ...
  async function handleCourseCreated({ subject }) {
    try {
      ...
      // 409 = user đã add môn này rồi (hiếm khi xảy ra vì modal đã lọc sẵn) — bỏ qua, vẫn cho vào folder.
      // Lỗi khác (network, 500...) → throw lại để modal hiện thông báo, không đóng modal.
  ...
  // Bấm vào 1 mã môn trong dropdown "Delete" của action menu — mở ConfirmDialog
  function handleDeleteSubject(subject) {
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        // Không dùng finally để đóng dialog — chỉ đóng khi thành công.
        // Nếu BE trả lỗi, exception sẽ bubble lên ConfirmDialog để hiển thị trạng thái lỗi,
        // và dialog vẫn mở để user biết xóa thất bại (không âm thầm mất đi).
        await userSubjectApi.remove(userId, subject.subjectId);

        // Cập nhật state ngay sau khi BE xác nhận xóa thành công
        setAddedSubjects((prev) =>
        ...
        // Đóng dialog sau khi state đã update — chỉ chạy nếu không có lỗi
                      ...
                      >
                        {subjects.map((us) => (
                          <a
                            ...
                            onClick={(e) => {
                              e.preventDefault();
                              navigate("/student/library/" + us.subjectName);
                            }}
